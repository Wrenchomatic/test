...
using namespace cv;
/*
This file contains the modified code of the ProjectionExercise
*/
...
//Lidar rotation relative to camera, rvecE = euler. The coefficients we got from Calibrate2 are added here
double rv[3];
rv[0] = -0.04;
rv[1] = -0.014;
rv[2] = 0.026;
...
K.at<double>(0, 0) = 1350; //Close to the real focus distance
K.at<double>(0, 1) = 0.;
K.at<double>(0, 2) = 640.;//960.;

K.at<double>(1, 0) = 0.;
K.at<double>(1, 1) = 1350;
...
distCoeffs.at<double>(0) = -3.4246837776676697e-01;
distCoeffs.at<double>(1) = -1.997298870530597e-01;
distCoeffs.at<double>(2) = 0.;
distCoeffs.at<double>(3) = 0.;
distCoeffs.at<double>(4) = 1.7503415283764048e-01;
  ...
  for(;;)
  {
	...
	//This box needs to be saved in Rect object in projection_objects list
		...
		//Go through all points to find out which object it belongs to
        for(int i = 0; i < points.size(); i++)
        {
                int closeIndex = 0;                                    
				// Assuming that the points are closest to the first object by default
                double leastDistance = distance(objects[0], points[i]); 

                for ( int j = 1; j < objects.size(); j++) // For every object
                {
                        double temp = distance(objects[j], points[i]);
                        if ( leastDistance > temp) // if the point is closer to the first object than another object
                        {
                                //A new close object has been found
                                closeIndex = j;         
								// this is the point -> object mapping (projection_objects[closeIndex])
								// temporary variable to find the closest object for each point
                                leastDistance = temp;   
                        }
                }
                if( leastDistance >= 100.0 ) // If the point is not close to any object ==> do nothing
                {

                }
                else
                {
                        // Modify the drawn rectangles to encompass the projected points
                        // Modify y-axis
                        
                        // if the rectangle hasn't been modified, set the object y to the point y
                        if( projection_objects[closeIndex].y == 0 && projection_objects[closeIndex].x == 0)
                        {
                                projection_objects[closeIndex].y = projection_points[i].y;
                                projection_objects[closeIndex].x = projection_points[i].x;


                        }
                        // Rectangle is not high enough
                        if( projection_objects[closeIndex].y > projection_points[i].y)
                        {
                                // must increment y coordinate and increase height by same amount
                                int diff = projection_objects[closeIndex].y - projection_points[i].y;
                                projection_objects[closeIndex].y -= diff;
                                projection_objects[closeIndex].height += diff;
                                
                                
                        }
                        // Rectangle is not low enough
                        if ( projection_objects[closeIndex].y + projection_objects[closeIndex].height < projection_points[i].y)
                        {
                                int diff = projection_points[i].y - projection_objects[closeIndex].y - projection_objects[closeIndex].height;
                                projection_objects[closeIndex].height += diff;
                        }
                        ...
                        // Rectangle is not wide enough left side
                        if( projection_objects[closeIndex].x > projection_points[i].x)
                        {
                                // Must move x coordinate and increase width by same amount
                                int diff = projection_objects[closeIndex].x - projection_points[i].x;
                                projection_objects[closeIndex].x -= diff;
                                projection_objects[closeIndex].width += diff ;
                                
                                
                        }
                        // Rectangle is not wide enough right side
                        if ( projection_objects[closeIndex].x + projection_objects[closeIndex].width < projection_points[i].x)
                        {
                                int diff = projection_points[i].x - projection_objects[closeIndex].x - projection_objects[closeIndex].width;
                                projection_objects[closeIndex].width += diff;
                        }
                }
        ...
        //Draw the rectangles, Added color to make the lines more visible
		//Also make the rectangle bigger
        for ( int j = 0; j < objects.size(); j++)
        {
                int color = (50 * j * 255/150) % 255; // Make colors for every different object
                cv::rectangle(frame, cv::Rect(projection_objects[j].x -10, projection_objects[j].y-10, projection_objects[j].width+20, projection_objects[j].height+20), Scalar(color, 0, 255 - color), 2);        
        }
